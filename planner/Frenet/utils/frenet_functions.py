...
from Cog_Dec_Mak.risk_assessment.risk_costs import calc_Cog_risk, generate_reward_space
...
class state_space:
    def __init__(
        ...
        self.s = s
        self.d = d
        self.width = (2 * d) * col / (col - 1)
        self.length = s * raw / (raw - 1)
        self.grid_size_lat = self.width / col
        self.grid_size_log = self.length / raw
...
def calc_frenet_trajectories(
    ...
    for vT in v_list:
        ...
        for tT in t_list:
            ...
            # find the maximum ds for the s-d frame
            if ds >= s_max:
                s_max = ds
...
def sort_frenet_trajectories(
    ...
    if predictions is not None:
        ...
        # initialize a reward space
        reward_space = np.zeros((space.raw, space.col))

        '''
        # generate cognitive matrix
        B = generate_B_matrices(space.raw, space.col)
        avg_B = np.sum(B, axis=2) / len(B[:,:,0])
        I = np.identity(space.raw * space.col)
        ...
        for fp in fp_list:
            ...
            # generate a reward space
            reward_space, fp.obst_risk_dict, fp.ego_harm_dict, fp.obst_harm_dict, keys = generate_reward_space(                
                traj=fp,
        ...
        for fp in fp_list:
            ...
            fp.ego_risk_dict, fp.bd_harm = calc_Cog_risk(
                traj=fp,
                ego_state=ego_state,
...
def get_v_list(
    ...
    """
    Get a list of end velocities for the frenét planner.   # 为planner去计算结束速度
