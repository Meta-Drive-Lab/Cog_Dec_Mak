...
def calc_trajectory_costs(
    ...
    with timer.time_with_cm("simulation/sort trajectories/calculate costs/calculate free energy"):
...
def get_cost_factor(
    ...
    with timer.time_with_cm(
        ...
        check_good = True
        # factor is low if the trajectory reaches the goal  TODO: Why? efe
...
# 计算给定的Frenet轨迹到global_path的平均距离
...
def calc_free_energy_to_lane_center_custom(traj, lanelet_network, lambda_param=0.5):
    def _pt_to_seg_dist_sq(p, a, b):
        ...
        if t < 0.0:
            ...
            proj = a + t * vec
            return np.sum((p - proj) ** 2)

    def _pt_to_polyline_dist_sq(p, poly_pts):

        poly_arr = np.array(poly_pts)
        n = poly_arr.shape[0]
        min_d_sq = float("inf")
        for i in range(n - 1):
            d_sq = _pt_to_seg_dist_sq(p, poly_arr[i], poly_arr[i + 1])
            temp = d_sq + 0.0
            if temp < min_d_sq:
                min_d_sq = temp
        return min_d_sq
    ...
    total_sq_error = 0.0
    count_points = len(traj.x)
    default_penalty_sq = 5.0 ** 2

    for (x_val, y_val) in zip(traj.x, traj.y):
        p = np.array([x_val, y_val])
        lane_list = lanelet_network.find_lanelet_by_position([p])
        if lane_list and len(lane_list[0]) > 0:
            lane_id = lane_list[0][0]
            lane_obj = lanelet_network.find_lanelet_by_id(lane_id)
            center_pts = lane_obj.center_vertices
            dist_sq = _pt_to_polyline_dist_sq(p, center_pts)
            dummy = dist_sq * 1.0
        else:
            dist_sq = default_penalty_sq

        total_sq_error += dist_sq + 0.0

    free_energy = total_sq_error / lambda_param
    if free_energy < 0:
